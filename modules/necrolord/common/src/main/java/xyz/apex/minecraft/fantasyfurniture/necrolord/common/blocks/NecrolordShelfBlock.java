...
public final class NecrolordShelfBlock extends BaseBlockComponentHolder
{
    private static final VoxelShapeCacher SHAPE_CACHE = new VoxelShapeCacher(blockState -> FantasyFurniture.getCorrectedShape(blockState, switch(blockState.getValue(ModBlockStateProperties.SHELF_TYPE)) {
        case LEFT -> NecrolordVoxelShapes.SHELF_LEFT;
        case RIGHT -> NecrolordVoxelShapes.SHELF_RIGHT;
        case SINGLE -> NecrolordVoxelShapes.SHELF_SINGLE;
        case CENTER -> NecrolordVoxelShapes.SHELF_CENTER;
    }));

    public NecrolordShelfBlock(Consumer<Registrar> registrarConsumer, Properties properties)
    {
