...
public class NecrolordFloorLightBlock extends BaseBlockComponentHolder
{
    private static final VoxelShapeCacher SHAPE_CACHE = new VoxelShapeCacher(blockState -> FantasyFurniture.getCorrectedShape(blockState, NecrolordVoxelShapes.FLOOR_LIGHT));

    public NecrolordFloorLightBlock(Consumer<Registrar> registrarConsumer, Properties properties)
    {
    ...
    public void animateTick(BlockState blockState, Level level, BlockPos pos, RandomSource rng)
    {
        ...
        level.addParticle(ParticleTypes.FLAME, x, y + .2D, z, 0D, 0D, 0D);

        level.addParticle(ParticleTypes.SMOKE, x + (stepX * offsetH), y + .1D, z + (stepZ * offsetH), 0D, 0D, 0D);
        level.addParticle(ParticleTypes.FLAME, x + (stepX * offsetH), y + .1D, z + (stepZ * offsetH), 0D, 0D, 0D);
