...
public final class NecrolordBedBlock extends BaseBlockComponentHolder
{
    private static final VoxelShapeCacher SHAPE_CACHE = new VoxelShapeCacher(blockState -> {
        VoxelShape shape;

        if(NecrolordFurnitureSet.Blocks.BED_SINGLE.hasBlockState(blockState)) shape = NecrolordVoxelShapes.BED_SINGLE;
        else if(NecrolordFurnitureSet.Blocks.BED_DOUBLE.hasBlockState(blockState)) shape = NecrolordVoxelShapes.BED_DOUBLE;
        else return Shapes.block();

        return FantasyFurniture.getCorrectedShape(blockState, shape);
    });

    public NecrolordBedBlock(Consumer<Registrar> registrarConsumer, Properties properties)
    {
