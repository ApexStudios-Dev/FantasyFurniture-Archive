...
public final class RoyalBedBlock extends RoyalDyeableBlock
{
    private static final VoxelShapeCacher SHAPE_CACHE = new VoxelShapeCacher(blockState -> {
        VoxelShape shape;

        if(RoyalFurnitureSet.Blocks.BED_SINGLE.hasBlockState(blockState)) shape = RoyalVoxelShapes.BED_SINGLE;
        else if(RoyalFurnitureSet.Blocks.BED_DOUBLE.hasBlockState(blockState)) shape = RoyalVoxelShapes.BED_DOUBLE;
        else return Shapes.block();

        return FantasyFurniture.getCorrectedShape(blockState, shape);
    });

    public RoyalBedBlock(Consumer<Registrar> registrarConsumer, Properties properties)
    {
