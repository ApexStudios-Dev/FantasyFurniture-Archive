...
public final class DunmerShelfBlock extends BaseBlockComponentHolder
{
    private static final VoxelShapeCacher SHAPE_CACHE = new VoxelShapeCacher(blockState -> FantasyFurniture.getCorrectedShape(blockState, switch(blockState.getValue(ModBlockStateProperties.SHELF_TYPE)) {
        case LEFT -> DunmerVoxelShapes.SHELF_LEFT;
        case RIGHT -> DunmerVoxelShapes.SHELF_RIGHT;
        case SINGLE -> DunmerVoxelShapes.SHELF_SINGLE;
        case CENTER -> DunmerVoxelShapes.SHELF_CENTER;
    }));

    public DunmerShelfBlock(Consumer<Registrar> registrarConsumer, Properties properties)
    {
