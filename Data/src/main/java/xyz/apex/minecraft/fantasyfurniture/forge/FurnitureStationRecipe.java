...
{
    private static final int SLOT_INGREDIENT_LEFT = 0;
    private static final int SLOT_INGREDIENT_RIGHT = 1;
    private static final int SLOT_BINDING_AGENT = 2;
    private static final int SLOT_INGREDIENT_COUNT = 3;
    ...
    {
        if(container.getContainerSize() >= SLOT_INGREDIENT_COUNT)
        {
            var inputLeft = container.getItem(SLOT_INGREDIENT_LEFT);
            var inputRight = container.getItem(SLOT_INGREDIENT_RIGHT);
            var inputBinding = container.getItem(SLOT_BINDING_AGENT);
    ...
    public ItemStack assemble(Container container)
    {
        if(container.getContainerSize() >= SLOT_INGREDIENT_COUNT)
        {
            if(!ingredientLeft.test(container.getItem(SLOT_INGREDIENT_LEFT))) return ItemStack.EMPTY;
            if(!ingredientRight.test(container.getItem(SLOT_INGREDIENT_RIGHT))) return ItemStack.EMPTY;
            if(!bindingAgent.test(container.getItem(SLOT_BINDING_AGENT))) return ItemStack.EMPTY;
    ...
    {
        return width * height >= SLOT_INGREDIENT_COUNT;
    }
    ...
    {
        return FantasyFurnitureDataMod.FURNITURE_STATION_RECIPE_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType()
    {
        return FantasyFurnitureDataMod.FURNITURE_STATION_RECIPE_TYPE.get();
    }
    ...
    {
        return new ItemStack(FantasyFurnitureDataMod.FURNITURE_STATION_BLOCK.get());
    }
    ...
    public static final class Serializer implements RecipeSerializer<FurnitureStationRecipe>
    {
        ...
        public FurnitureStationRecipe fromJson(ResourceLocation recipeId, JsonObject json)
        {
            ...
            var result = Objects.requireNonNull(ForgeRegistries.ITEMS.getValue(new ResourceLocation(GsonHelper.getAsString(json, "result"))));
        ...
        public FurnitureStationRecipe fromNetwork(ResourceLocation recipeId, FriendlyByteBuf data)
        {
            ...
            var result = Objects.requireNonNull(ForgeRegistries.ITEMS.getValue(data.readResourceLocation()));
            return new FurnitureStationRecipe(recipeId, group, ingredientLeft, ingredientRight, bindingAgent, result);
        }
        ...
        public void toNetwork(FriendlyByteBuf data, FurnitureStationRecipe recipe)
        {
            ...
            data.writeResourceLocation(Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(recipe.result.asItem())));
        }
    ...
    private static final class CompiledRecipe implements FinishedRecipe
    {
        ...
        public void serializeRecipeData(JsonObject json)
        {
            ...
            json.addProperty("result", Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(result.asItem())).toString());
        }
